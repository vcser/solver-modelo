...
void GreedyBaseline::solve()
{
  ...
  while (!colaIncendios.empty())
  {
    ...
    {
      // ta mal
      std ::cerr << "No se pudo apagar el incendio " << actual.getId() << " reintentando con otra asignacion " << std::endl;
      repitoMismoIncendio = true;
    }
    else
    {
      // std ::cerr << "El incendio " << actual.getId() << " se pudo apagar con la asignacion con t=" << apagado << std::endl;
      // std::cerr << apagado - actual.getTimestampInicio() << " segundos desde el inicio" << std::endl;
      colaIncendios.pop();
...
int GreedyBaseline::calcularPrioridad(const Incendio &incendio)
{
  ...
  debugOut << "Distancia a ciudad mas cercana incendio " << incendio.getId() << " " << distanciaCiudadMasCercana << std::endl;
  int costo;
  debugOut << "Prioridad de incendio " << incendio.getId() << std::endl;
  if (distanciaCiudadMasCercana <= radioDeIncendio)
  {
    debugOut << "Infinita porque distancia Ciudad mas cercana es " << distanciaCiudadMasCercana << " y radio de incendio es " << radioDeIncendio << std::endl;
    costo = INT_MAX;
  }
  else
  {
