...
{
public:
  DataChunkFlat(size_t size) : data(size) {}
  DataChunkFlat(size_t size, double init) : data(size, init) {}
  DataChunkFlat(void) {}

  std::vector<double> &get_1d_rw() { return data; }
  std::vector<double> const &get_1d() const { return data; }
  void set_data(std::vector<double> const &d) { data = d; };
  size_t get_data_dim(void) const { return 1; }

  void show_name()
  {
    std::cout << "DataChunkFlat " << data.size() << std::endl;
  }
  void show_values()
  {
